/*
Surreal Shapes Spline Curve Slider
...
var vbrgb = [0.1, 0.1, 0.1, 1.]; // Dark background color
var vfrgb = [0.8, 0.2, 0.5, 1.]; // Bright foreground color
var vrgb2 = [0.2, 0.8, 0.2, 1.]; // Vibrant line color
var vmode = 0; // Marker mode (0: rectangle, 1: ellipse, 2: triangle, 3: star)
...
function draw() {
    ...
    with (sketch) {
        ...
        for (var i = -1; i <= 1; i += 0.1) {
            var y = Math.sin(i * Math.PI * 3 + val * Math.PI * 2) * 0.3; // Wavy effect
            lineto(i, y);
        }
        if (voutline) {
            glcolor(0, 0, 0);
...
function drawStar(x, y, radius) {
    ...
    for (var i = 0; i < points * 2; i++) {
        var r = (i % 2 === 0) ? radius : radius * 0.5; // Alternate radius
        var theta = i * angle;
